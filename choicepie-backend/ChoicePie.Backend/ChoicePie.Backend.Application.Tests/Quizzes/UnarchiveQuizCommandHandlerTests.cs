...
public class UnarchiveQuizCommandHandlerTests
{
    ...
    public void SetUp()
    {
        ...
        _sut = new UnarchiveQuizCommandHandler(_quizRepository, _memberRepository, _currentUserService, _unitOfWork);
    ...
    [Test]
    public async Task Handle_GivenOwner_WhenCalled_ThenUnarchivesAndPersists()
    {
        var result = await _sut.Handle(new UnarchiveQuizCommand(_quiz.Id), CancellationToken.None);
    ...
    public void Handle_GivenNonOwner_WhenCalled_ThenThrowsQuizForbiddenException()
    {
        ...
        Assert.ThrowsAsync<QuizForbiddenException>(() => _sut.Handle(new UnarchiveQuizCommand(_quiz.Id), CancellationToken.None));
    }
}
