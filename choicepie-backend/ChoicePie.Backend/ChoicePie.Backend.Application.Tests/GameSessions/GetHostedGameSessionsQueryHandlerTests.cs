...
[TestFixture]
public class GetHostedGameSessionsQueryHandlerTests
{
    private IGameSessionQueryService _gameSessionQueryService = null!;
    private ICurrentUserService _currentUserService = null!;
    private GetHostedGameSessionsQueryHandler _sut = null!;
    ...
    public void SetUp()
    {
        ...
        _sut = new GetHostedGameSessionsQueryHandler(_gameSessionQueryService, _currentUserService);

        _currentUserService.UserId.Returns(_userId);
    }

    [Test]
    public async Task Handle_GivenAuthenticatedUser_WhenCalled_ThenReturnsHostedSessionsForCurrentUser()
    {
        var expected = new PagedResult<GameSessionSummaryDto>([], 1, 20, 0);
        _gameSessionQueryService.GetHostedByUserIdAsync(_userId, 1, 20, Arg.Any<CancellationToken>()).Returns(expected);

        var result = await _sut.Handle(new GetHostedGameSessionsQuery(1, 20), CancellationToken.None);
    ...
    public void Handle_GivenNoCurrentUser_WhenCalled_ThenThrowsUnauthenticatedException()
    {
        ...
        Assert.ThrowsAsync<UnauthenticatedException>(() =>
            _sut.Handle(new GetHostedGameSessionsQuery(1, 20), CancellationToken.None));
    }
}
