...
public sealed class ListCommentsByQuizIdQueryHandler(
    ...
    : IRequestHandler<ListCommentsByQuizIdQuery, PagedResult<CommentDto>>
{
    public async Task<PagedResult<CommentDto>> Handle(ListCommentsByQuizIdQuery request, CancellationToken cancellationToken)
    {
        _ = await quizRepository.GetByIdAsync(request.QuizId, cancellationToken)
            ?? throw new QuizNotFoundException(request.QuizId);

        return await commentQueryService.ListByQuizIdAsync(
            request.QuizId, request.PageNumber, request.PageSize, cancellationToken);
    }
}
