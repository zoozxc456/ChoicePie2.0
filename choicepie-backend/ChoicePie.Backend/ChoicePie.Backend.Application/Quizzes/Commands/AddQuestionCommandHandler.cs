...
public sealed class AddQuestionCommandHandler(
    ...
    : IRequestHandler<AddQuestionCommand, QuizDto>
{
    public async Task<QuizDto> Handle(AddQuestionCommand request, CancellationToken cancellationToken)
    {
        ...
        quiz.AddQuestion(Question.Create(request.Text, request.Options, request.AnswerIndex, request.Explanation));
